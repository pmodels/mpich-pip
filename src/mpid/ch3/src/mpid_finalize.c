...
extern double lmt_pip_prof_gen_datatype_timer;
extern int lmt_pip_prof_lmt_gen_datatype_cnt;
extern int lmt_pip_prof_noncontig_nchunks;
extern int lmt_pip_prof_lmt_noncontig_cnt;

static void lmt_pip_prof_print(void) {
    int rank = 0;
    double avg_gen_datatype_timer = 0.0;
    int avg_noncontig_nchunks = 0;

    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    if (lmt_pip_prof_lmt_gen_datatype_cnt > 0)
        avg_gen_datatype_timer = lmt_pip_prof_gen_datatype_timer / lmt_pip_prof_lmt_gen_datatype_cnt;
    if (lmt_pip_prof_lmt_noncontig_cnt > 0)
        avg_noncontig_nchunks = lmt_pip_prof_noncontig_nchunks / lmt_pip_prof_lmt_noncontig_cnt;

    fprintf(stdout, "[%d] PIP_LMT_PROF: avg gen_datatype_timer %.4lf, cnt %d\n",
            rank, avg_gen_datatype_timer * 1000 * 1000, lmt_pip_prof_lmt_gen_datatype_cnt);
    fprintf(stdout, "[%d] PIP_LMT_PROF: avg noncontig_nchunks %d, cnt %d\n",
            rank, avg_noncontig_nchunks, lmt_pip_prof_lmt_gen_datatype_cnt);
    fflush(stdout);
}
#endif
